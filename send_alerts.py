    ...
    for alert in Alert.objects.all():
        ...
        if not alert.sent:
            ...
            if index < len(all_items):
                ...
                if all_items[index]['number'] == alert.item.number:
                    # notify the subscriber that the Tulsa City Council is now on their agenda item
                    message = client.messages.create(to=alert.phone, from_="9185508625", 
                        body="Your Agenda Item : %d is up!" % current_item)
                    # mark the alert as sent
                    alert.sent = True
                    # update the alert
                    alert.save()
                # check if the subscribed item is the next item
                else if index+1 < len(all_items) and all_items[index+1]['number'] == alert.item.number:
                    # notify the subscriber that their agenda item is the next item
                     message = client.messages.create(to=alert.phone, from_="9185508625", 
                        body="Tulsa City Council is now on Agenda Item : %d. Your Agenda Item is next!" % current_item)
                # check if the subscribed item is the second to next item
                else if index+2 < len(all_items) and all_items[index+2]['number'] == alert.item.number:
                    # notify the subscriber that their agenda item is the second to next item
                     message = client.messages.create(to=alert.phone, from_="9185508625", 
                        body="Tulsa City Council is now on Agenda Item: %d. Your Agenda Item is 2 away!" % current_item)
